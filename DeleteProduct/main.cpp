...
class ProductHandler {
   public:
    ...
    invocation_response handler(Aws::String id) { return removeProduct(id); }

   private:
    invocation_response removeProduct(Aws::String id) {
        ...
        request.SetReturnValues(Aws::DynamoDB::Model::ReturnValue::ALL_OLD);

        const auto outcome = client.DeleteItem(request);

        if (outcome.IsSuccess()) {
            if (!outcome.GetResult().GetAttributes().empty()) {
                JsonValue response;
                response.WithString("body", "Product has been deleted");
